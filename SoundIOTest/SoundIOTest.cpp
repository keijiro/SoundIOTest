    ...
    while (frames_left > 0) {
        ...
        for (int frame = 0; frame < frame_count; frame += 1) {
            for (int ch = 0; ch < outstream->layout.channel_count; ch += 1) {
                ...
                *((float*)areas[ch].ptr) = my_filter->do_sample(*((float*)read_ptr));
                areas[ch].ptr += areas[ch].step;
...
//    my_filter = new Filter(LPF, 51, 48, 0.2);
//    my_filter = new Filter(HPF, 51, 48, 2);
    my_filter = new Filter(BPF, 51, 48, 0.5, 1.5);
